...
# --- NEW: Setup SpaceX and their accounts ---
puts "\n6. Setting up SpaceX and their accounts..."
...
# Find or create SpaceX's GCC Account
spacex_gcc_account = Account.find_or_create_for_entity_and_currency(
  accountable_entity: spacex,
  currency: gcc_currency
)
puts "✅ SpaceX's GCC Account: ID #{spacex_gcc_account.id}, Balance: #{spacex_gcc_account.balance.to_f} GCC"

# Find or create SpaceX's USD Account
spacex_usd_account = Account.find_or_create_for_entity_and_currency(
  accountable_entity: spacex,
  currency: usd_currency
)
puts "✅ SpaceX's USD Account: ID #{spacex_usd_account.id}, Balance: #{spacex_usd_account.balance.to_f} USD"
...
# --- NEW: LDC Paying SpaceX ---
puts "\n12. LDC paying SpaceX for supplies..."
cost_in_gcc = 250.0
cost_in_usd = 150.0
...
begin
  ...
  puts "✅ LDC paid #{cost_in_gcc} GCC to SpaceX. LDC GCC Balance: #{ldc_gcc_account.balance.to_f} GCC"
  puts "   SpaceX GCC Balance: #{spacex_gcc_account.reload.balance.to_f} GCC"
rescue => e
...
if ldc_usd_account.balance < cost_in_usd
  puts "⚠️ LDC USD balance too low to pay SpaceX in USD. Current: #{ldc_usd_account.balance.to_f} USD, Needed: #{cost_in_usd} USD. Adding funds for test."
...
begin
  ...
  puts "✅ LDC paid #{cost_in_usd} USD to SpaceX. LDC USD Balance: #{ldc_usd_account.balance.to_f} USD"
  puts "   SpaceX USD Balance: #{spacex_usd_account.reload.balance.to_f} USD"
rescue => e
...
# Reload SpaceX accounts to show final balances
spacex_gcc_account.reload
spacex_usd_account.reload
puts "  - SpaceX GCC Account Balance: #{spacex_gcc_account.balance.to_f} #{spacex_gcc_account.currency.symbol}"
puts "  - SpaceX USD Account Balance: #{spacex_usd_account.balance.to_f} #{spacex_usd_account.currency.symbol}"
