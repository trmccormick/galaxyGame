...
# --- Custom Stub for Exchange Rate Service ---
# This simulates the real service and allows rate manipulation for the test scenario.
class ExchangeRateService
  # Initializes with a map of rates: { ["FROM", "TO"] => rate }
  def initialize(initial_rates)
    @rates = initial_rates.transform_keys { |k| k.map(&:upcase) }
  end

  # Allows the test to manually set the market rate (e.g., 1 USD = 1.3 GCC)
  def set_rate(from_currency_symbol, to_currency_symbol, rate)
    key = [from_currency_symbol.upcase, to_currency_symbol.upcase]
    @rates[key] = rate
    puts "    [MARKET] Exchange Rate updated: 1 #{from_currency_symbol} = #{rate.round(2)} #{to_currency_symbol}."
  end

  def convert(amount, from_currency_symbol, to_currency_symbol)
    rate = @rates[[from_currency_symbol.upcase, to_currency_symbol.upcase]]
    if rate
      (amount * rate).round(2)
    else
      # Fallback to 1:1 if rate isn't found (shouldn't happen for USD/GCC)
      amount.round(2)
    end
  end
end
...
# Assuming Game and GameState models are defined and accessible via ActiveRecord/Rails
game = Game.new
...
# === 12. Simulate Game Days Mining (and **Currency Fluctuation**) ===
puts "\n12. Simulating mining over game days using Game loop..."

# Initialize Exchange Rate Service (assuming 1 USD = 1 GCC initially based on our conversations)
exchange_service = ExchangeRateService.new({ ["USD", "GCC"] => 1.0 })
...
game_days.times do |day|
  ...
  # --- CRITICAL SIMULATION POINT ---
  # On the last day, simulate the GCC currency depreciating against the USD.
  # This demonstrates the risk of the USD-denominated bond from step 9.
  if day == game_days - 1
    # 1 USD now costs 1.3 GCC (USD appreciated, GCC depreciated)
    exchange_service.set_rate("USD", "GCC", 1.30)
  end
  # ---------------------------------
  ...
  # sleep(0.01) # Use for slow execution, commented out for fast simulation
end
...
bonds.each do |bond|
  ...
  if current_date >= bond.due_at
    # Convert USD bond amount to GCC using the current (fluctuated) rate
    gcc_amount = exchange_service.convert(bond.amount, "USD", "GCC").round(2)
    available_gcc = ldc_gcc_account.balance.to_f

    puts "    Repayment Check for Bond ##{bond.id} (USD Debt: #{bond.amount} USD)"
    puts "    Required GCC Repayment at today's rate: #{gcc_amount} GCC"
...
begin
  if defined?(MiningLog) && MiningLog.table_exists?
