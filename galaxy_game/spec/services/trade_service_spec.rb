...
RSpec.describe TradeService do
  let(:colony) { create(:colony, :with_multiple_settlements, name: 'Colony A') }
  let(:buyer_colony) { create(:colony, :with_multiple_settlements, name: 'Colony B') }
  let(:inventory) do
    inv = create(:inventory, inventoryable: colony)
    create(:item, inventory: inv, name: 'Iron', amount: 500, material_type: 'raw_material')
    inv
  end
  ...
  describe '#base_price_for_type' do
    ...
    it 'returns the correct base price for processed goods' do
      inventory.items.first.update(material_type: 'processed_material')
      trade_service = TradeService.new(inventory, buyer_colony)
  ...
  describe '#distance_to_buyer' do
    it 'calculates the distance between colonies' do
      # Mock the distance_from_star for testing
      allow_any_instance_of(CelestialBodies::CelestialBody).to receive(:distance_from_star).and_return(10000)
      allow(buyer_colony.celestial_body).to receive(:distance_from_star).and_return(5000)
