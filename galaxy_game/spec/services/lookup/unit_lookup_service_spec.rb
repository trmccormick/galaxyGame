...
RSpec.describe Lookup::UnitLookupService do
  ...
  around(:each) do |example|
    # Ensure no global stubs/mocks leak between tests
    RSpec::Mocks.with_temporary_scope { example.run }
  end

  before(:all) do
    # Ensure test isolation: reload constants if needed
    if defined?(GalaxyGame::Paths)
      GalaxyGame::Paths.constants.each do |const|
        GalaxyGame::Paths.send(:remove_const, const)
      end
      load Rails.root.join('config', 'initializers', 'game_data_paths.rb')
    end
  end
  ...
  describe '#find_unit' do
    ...
    context 'with propulsion units' do
      ...
      it 'finds propulsion units by their exact ID' do
        # Use the first available propulsion unit
      ...
      it 'has expected propulsion unit properties' do
        ...
        # Check for performance data if it exists
  ...
  describe 'unit data structure validation' do
    context 'when any propulsion unit exists' do
      ...
      it 'has performance properties if applicable' do
        # Only check if performance data exists
