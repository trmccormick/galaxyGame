...
RSpec.describe Construction::DomeService, type: :service do
  ...
  before do
    ...
    allow(Construction::DomeCalculator).to receive(:calculate_materials).and_return({
    ...
    allow(Construction::DomeCalculator).to receive(:calculate_construction_cost).and_return(50000)
    allow(Construction::DomeCalculator).to receive(:estimate_construction_time).and_return(480)
  ...
  describe "#schedule_construction" do
    ...
    context "when entity is different from service provider" do
      ...
      it "does not create material requests for contracted construction" do
        expect(MaterialRequestService).not_to receive(:create_material_requests_from_hash)
  ...
  describe "#track_progress" do
    ...
    it "releases equipment when construction completes" do
      allow(Construction::ConstructionManager).to receive(:complete?).and_return(true)
      expect(Construction::EquipmentManager).to receive(:release_equipment).with(construction_job)
