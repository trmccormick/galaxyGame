...
RSpec.describe Financial::Account, type: :model do
  # Create a shared currency for all accounts in this test
  ...
  # Create accounts explicitly with the same currency
  let(:player_account) { create(:financial_account, accountable: player, currency: currency, balance: 1_000) }
  let(:recipient_account) { create(:financial_account, accountable: recipient_player, currency: currency, balance: 500) }
  ...
  describe 'different accountable types' do
    it 'works with a player account' do
      account = create(:financial_account, accountable: create(:player))
      expect(account.accountable).to be_a(Player)
    end
    
    it 'works with a settlement account' do
      account = create(:financial_account, accountable: create(:base_settlement))
      expect(account.accountable).to be_a(Settlement::BaseSettlement)
    end
    
    it 'works with an organization account' do
      account = create(:financial_account, accountable: create(:organization))
    ...
    #   account = create(:financial_account, accountable: colony)
