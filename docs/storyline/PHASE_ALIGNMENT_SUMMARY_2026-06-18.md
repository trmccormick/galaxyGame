...
- `phase6+/` = Luna worldhouse/lavatube buildout, live ISRU calibration, Earth import management
- `phase7+/` = L1/LEO Depot construction, gas processing pipeline establishment
- `phase8+/` = L1 station growth, shipyard construction, tug/cycler production
- `phase9+/` = Phobos/Deimos repositioning, hollowing prep, initial station fit-out, first cycler return/resupply loop
- `phase10+/` = Venus repeat of the Mars foothold loop, including a parallel second cycler-tug pair at Earth/L1
- `phase11+/` = normal Earth→Mars→Venus cycler logistics, docking/undocking, cargo transfer validation, and heavy logistics testing
- `phase12+/` = optional split for parallel branch expansion: Mars→Ceres belt mining and Mars→Titan/Saturn settlement
- `phase13+/` = Psyche asteroid mining (planetary core remnant) and parallel Mars terraforming initiation with Venus/Titan gas exports
- `phase14+/` = AI Manager operational independence test: Sol day-to-day management and initial Eden expansion
- `phase15+/` = Unplanned Eden expansion and natural wormhole mass-limit discovery, leading to the Snap crisis
...
**Phases 5–13 constitute the AI Manager training and infrastructure code testing layer**, not true in-world gameplay or permanent buildup:
...
**Wormhole Trigger Mechanism**: Once the AI Manager independently and consistently manages:
- Luna settlement initialization and supply chains
- Earth-Mars-Venus-Psyche core-loop logistics and cargo transfer
- Multi-world resource optimization and atmospheric engineering

A **game trigger event** spawns the natural wormhole. This ensures the AI Manager has learned foundational patterns before expanding to extrasolar systems. True multi-system expansion and wormhole-era gameplay then begin.

**Override for future triage**: If any older table or note below conflicts with the detailed ladder above, use the corrected order `phase6+` worldhouse, `phase7+` depots, `phase8+` shipyards/tug-cycler production, `phase9+` Mars footholds, `phase10+` Venus repeat loop, `phase11+` Earth-Mars-Venus cycler logistics, `phase12+` for optional Ceres/Titan-Saturn branch splits, `phase13+` for Psyche asteroid mining and Mars terraforming initiation, `phase14+` for AI Manager operational test and Eden expansion, and `phase15+` for unplanned Eden expansion and wormhole mass-limit discovery. The Snap crisis event occurs post-Phase 15, triggering Act 2 gameplay.
...
✅ **Backlog Classification Clear**: `phase6+/` = Luna worldhouse/ISRU/import work; `phase7+/` = depots; `phase8+/` = shipyards/tugs/cyclers; `phase9+/` = Mars footholds; `phase10+/` = Venus repeat loop; `phase11+/` = Earth-Mars-Venus logistics loop; `phase12+/` = optional Ceres/Titan-Saturn split; `phase13+/` = Psyche asteroid mining and Mars terraforming initiation; `phase14+/` = AI Manager operational test and Eden expansion; `phase15+/` = unplanned Eden expansion and wormhole mass-limit discovery  
