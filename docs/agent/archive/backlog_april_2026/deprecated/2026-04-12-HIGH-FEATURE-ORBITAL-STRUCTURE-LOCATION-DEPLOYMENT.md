# TASK: OrbitalStructure — CelestialLocation Created on Deployment (ARCHIVED)
**Status**: DEPRECATED — EXTRACTED TO ACTIONABLE PHASE 5+ & PHASE 6+ TASKS  
**Priority**: HIGH → N/A (split into multiple tasks with appropriate priorities)
**Type**: feature → SPLIT INTO REFACTOR_FEATURE + FUTURE ENHANCEMENTS
**Created**: 2026-04-12
**Last Updated**: 2026-06-08 (archived during backlog triage, extracted to actionable tasks)

---

## ⚠️ ARCHIVE NOTE — DO NOT IMPLEMENT FROM THIS FILE

Original task requested that OrbitalStructure creates CelestialLocation on deployment. **Core requirement extracted as Phase 5+ prerequisite**, with future enhancements separated into distinct Phase 6+ tasks. This file preserved for historical reference only.

### What Was Extracted (Actionable Work Created)

**Phase 5+ Prerequisite Task — Blocks Gas Processing Pipeline:**
📄 `docs/new_agent/projects/galaxy_game/tasks/backlog/phase5+/2026-06-HIGH-FEATURE-STANDARDIZE-ORBITAL-STRUCTURE-DEPLOYMENT.md`  
Implements: after_create callback on OrbitalStructure, standard deployment service pattern (generalizes DepotAdapter), backward compatibility maintained

**Phase 6+ Enhancement Tasks — Future Scalability:**
📄 `docs/new_agent/projects/galaxy_game/tasks/backlog/phase5+/2026-06-MEDIUM-FEATURE-MULTI-STRUCTURE-ROUTING-PER-SETTLEMENT.md`  
Implements: Load balancing across multiple structures per settlement, routing logic based on craft type + cargo manifest

📄 `docs/new_agent/projects/galaxy_game/tasks/backlog/phase5+/2026-06-MEDIUM-FEATURE-DYNAMIC-ORBITAL-POSITION-SIMULATION.md`  
Implements: Time-based position calculation using orbital mechanics, eclipse period detection (optional)

### Why Split Into Multiple Tasks?
1. **Phase 5+ prerequisite** — Standard deployment pattern blocks gas processing pipeline at L1 Station and WormholeNavigator pathfinding costs
2. **Bridge convention still valid for MVP** — One structure per settlement works fine until orbital stations scale beyond initial deployment  
3. **Static positions acceptable for Phase 5** — Dynamic simulation only needed when eclipse periods affect energy availability or cycler routes need precise timing windows

### Implementation Evidence (For Reference)
See `docs/new_agent/projects/galaxy_game/status.md` — "Confirmed Architecture" section:
- OrbitalSettlement#location delegates to structures.first&.celestial_location ✅  
- DepotAdapter#create_depot proves pattern works but is isolated ❌ → needs generalization
