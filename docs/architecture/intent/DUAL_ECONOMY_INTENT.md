...
**Properties**:
  ...
  - System currency (`is_system_currency: true`)
**Usage**:
  - *USD is used exclusively for NPC and institutional Earth-side procurement, including Earth-to-Space imports, logistics contracts, and infrastructure fees. Players do not transact in USD directly.*
...
- *Dynamic tolls are modeled after the Panama Canal, with rates adjusted based on mass, anchor requirements, and Sabatier process offsets (see below).* 

**Sabatier 40% Offset**: *Wormhole and logistics maintenance costs are reduced by 40% when the Sabatier process is active, as per the Mars-Ceres Logistics Agreement (Lore).*

**Physical Realism: Gravitational Anchor Arrays**
- *All stabilized wormholes require Gravitational Anchor Arrays at endpoints. The primary anchor must be a planet or moon with mass $> 95\ M_{\oplus}$ (see [STARSIM_GENERATION_RULES.md](../logistics/STARSIM_GENERATION_RULES.md)). If insufficient mass is available, artificial stabilization is not possible.*
...
- [STARSIM_GENERATION_RULES.md](../logistics/STARSIM_GENERATION_RULES.md) - Anchor mass thresholds for wormhole stabilization
...
- `app/models/transport/wormhole.rb` - Includes anchor_mass, mass_limit, sabatier_active for physical/economic realism
