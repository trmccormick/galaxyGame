...
**Technologies**:
- **Gravitational Anchor Arrays**: Stabilize wormhole throats using massive planetary or lunar bodies as anchors. *Primary endpoint must be a planet or moon with sufficient mass to counteract the "Drag" of the wormhole throat. See [STARSIM_GENERATION_RULES.md](../logistics/STARSIM_GENERATION_RULES.md) for $M_{\oplus}$ (Earth-mass) thresholds.*
...
stabilization_cost = {
  anchor_mass: 0.5,      # Minimum $M_{\oplus}$ for endpoint anchor (see STARSIM_GENERATION_RULES.md)
  energy: 5e15,          # MJ for anchor array deployment
...
- **Mass-Gating Protocols**: *Artificial Portals require a `mass_limit` check in the WormholeNavigator service to prevent destabilization events.*
...
```ruby
# Wormhole governance cost model (Dynamic Maintenance Formula)
# Mars-Ceres Logistics Agreement (Lore): Sabatier process yields a 40% maintenance offset for wormhole infrastructure using in-situ resources.
#
#   Cost = Base * GravityOffset * SabatierReduction
#   SabatierReduction = 0.6 (40% offset)
governance_cost = {
  maintenance: stabilization_cost * GravityOffset * 0.6,    # Dynamic maintenance (see Mars-Ceres Logistics Agreement)
...
class Wormhole < ApplicationRecord
  ...
  attribute :anchor_mass, :decimal      # $M_{\oplus}$, mass of endpoint anchor
  attribute :is_artificial, :boolean    # true if portal is artificial
  attribute :mass_limit, :decimal       # maximum mass allowed per traversal
  attribute :sabatier_active, :boolean  # true if Sabatier offset applies
