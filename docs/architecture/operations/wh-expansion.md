...
## Technical Specs for Ruby/JSON Integration

The following fields should be included in mission profiles, AI logic, and relevant JSON data structures:

- `mass_displacement_coefficient`: Float. Represents the amount of wormhole stability lost per ton of cargo transiting the wormhole.
- `refocus_efficiency_rating`: Float (0.0–1.0). Indicates how much leaked EM the current Sat/WS array can recycle (e.g., 0.85 = 85% efficiency).
- `hammer_trigger_tonnage`: Integer or Float. The specific mass (in tons) required to force an immediate Snap event (Hammer Protocol trigger).

These fields enable precise simulation of wormhole stability, EM recapture, and Snap event triggers in both Ruby backend logic and mission profile JSONs.
