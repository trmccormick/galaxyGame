...
describe AIManager::SystemArchitect do
  describe 'Sabatier Refinements' do
    let(:architect) { AIManager::SystemArchitect.allocate }
    ...
    describe '#sabatier_units_active?' do
      it 'returns true when contract flag is set' do
        ...
        expect(architect.send(:sabatier_units_active?, 'SOL-AC-01')).to be_a(Boolean)
      end
    end
  end
end