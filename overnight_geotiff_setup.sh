...
TEMP_DIR="$PROJECT_ROOT/data/geotiff/temp"
PATTERNS_DIR="$PROJECT_ROOT/data/ai_patterns"
TEST_MAPS_DIR="$PROJECT_ROOT/data/test_maps"
...
if __FILE__ == $0
  ...
  output_dir = ARGV[1] || File.expand_path('../../../data/ai_patterns', __dir__)
...
class TestMapGenerator
  def generate_test_map
    # Load patterns
    patterns_file = File.expand_path('../../../ai_patterns/geotiff_patterns.json', __dir__)
...
echo "" >> "$STATUS_FILE"
echo "⏱️  Total Runtime: $(($(date +%s) - $(head -1 "$LOG_FILE" | cut -d' ' -f2 | tr -d '-: '))) seconds" >> "$STATUS_FILE"
